...
module type S = sig
  ...
  (** Fuzzy search scoring based on fzf
      ...
      - The case of characters are ignored unless there is at least one uppercase
        character in the query. This is similar to 'smart case' behaviour in fzf except
        that fzf requires an exact case match when there is any uppercase character and we
        just apply a score deduction.
      - Queries are split into sub patterns delimited by spaces. All sub patterns must
        match the item and their scores are combined. This is the default behaviour in fzf
        called extended mode.
...
module type Fuzzy_search = sig
  module Query : Query
  include S with module Query := Query
  module (Portable @@ portable) : S with module Query := Query
