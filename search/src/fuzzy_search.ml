...
module Query = struct
  type t : immutable_data =
    { queries : string iarray (* The pattern split by spaces *)
    ; raw : string
    ; case_sensitive : bool
    }
  [@@deriving equal]
...
(* [score] gets called a lot, so we want it to be zero-alloc. 
   ...
   But global singletons aren't portable, so we expose both a "always zero-alloc,
   non-portable" version, and a "portable, but zero-alloc only on native" version. *)
module type%template [@modality p = (portable, nonportable)] Start_end_indices = sig
  type t = private
    { mutable found : bool
...
module Start_end_indices_singleton : Start_end_indices = struct
  type t =
    { mutable found : bool
    ; mutable start_idx : int
    ; mutable end_idx : int
  ...
  let singleton = { found = false; start_idx = 0; end_idx = 0 }

  let set_and_return_singleton ~found ~start_idx ~end_idx =
    Fields.Direct.set_all_mutable_fields singleton ~found ~start_idx ~end_idx;
    singleton
  ;;
end

module Start_end_indices_portable : Start_end_indices [@modality portable] = struct
  type t =
    { mutable found : bool
    ; mutable start_idx : int
    ; mutable end_idx : int
  ...
  let set_and_return_singleton ~found ~start_idx ~end_idx = exclave_
    stack_ { found; start_idx; end_idx }
  ;;
end
...
module%template.portable
  [@inline] Parameterized_over_start_and_end_indices_acquisition
    (Start_end_indices : Start_end_indices) =
struct
  let[@inline] find_start_end_indices ~query ~item () = exclave_
    ...
    then Start_end_indices.set_and_return_singleton ~found:false ~start_idx:0 ~end_idx:0
    else (
  ...
  let matching_indices (query : Query.t) ~item =
    ...
    then Some [||]
      ...
      let indices =
        Iarray.fold query.queries ~init:Int.Set.empty ~f:(fun set query ->
  ...
  *)
  ...
  let score query ~item =
    ...
    | _, _ ->
      ...
      for i = 0 to Iarray.length queries - 1 do
        let query = Iarray.get queries i in
...
include Parameterized_over_start_and_end_indices_acquisition (Start_end_indices_singleton)

module Portable =
  Parameterized_over_start_and_end_indices_acquisition [@modality portable]
    (Start_end_indices_portable)

module For_testing = struct
  let find_start_end_indices ~query ~item () =
    match find_start_end_indices ~query ~item () with
    | { found = false; _ } -> None
    | { found = true; start_idx; end_idx } -> Some (start_idx, end_idx)
  ;;
end
